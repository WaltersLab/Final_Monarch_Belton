...
caption_text <- str_wrap(
  "Figure 5. This demonstrates the seperate and significant impact both sex and size has on aging. Panel A shows the plotted predicted values from the P4 model, under the assumption that the butterfly is of ‘average size’, to control the predictor of size, against the previously documented Kaplan-Meier curves. Panel B depicts P4 plotted separately by sex at three representative size values (categorized as small: one SD below mean size, average: mean size, and large: individuals one SD above mean size), showing that as size increased, aging was reduced (Fig. 6).",
  width = 90
)
